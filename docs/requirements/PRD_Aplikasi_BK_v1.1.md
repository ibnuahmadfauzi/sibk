...
Ketika data tahun ajaran baru belum tersedia di SIBK, Admin IT dapat menyiapkan tahun ajaran hanya dengan namanya, tanpa mengisi tanggal periode atau nomor SK. Daftar rombel aktif dikelola di menu Data Kelas dan otomatis tersedia pada tahun baru tanpa dipilih satu per satu. Admin IT mengimpor daftar minimum murid yang sudah tersedia dari URL API Siswa sekali pakai atau CSV fallback berdasarkan data sekolah. Koordinator BK melengkapi penugasan, termasuk pada rombel yang masih kosong, lalu dapat mengaktifkan tahun ajaran; murid baru yang datanya datang belakangan ditambahkan melalui impor yang sama pada tahun aktif. Penempatan mengikuti rombel dari sumber, termasuk murid tinggal kelas, tanpa kenaikan otomatis. Murid tahun sebelumnya yang belum memiliki penempatan pada tahun target ditampilkan sebagai daftar pemeriksaan tanpa menebak status akademik dan tanpa menghalangi aktivasi murid lain. Data persiapan sementara selalu diberi penanda sampai diverifikasi dari sumber resmi.
...
| Data persiapan sementara | Admin IT menyiapkan tahun ajaran, rombel, dan daftar minimum `nisn,nama,rombel,tahun_pelajaran` dari URL API Siswa sekali pakai atau CSV fallback. Satu impor dapat memuat beberapa tahun sementara yang sudah dibuat, termasuk yang sudah aktif. Data diberi penanda sampai diverifikasi dari sumber resmi. |
...
| Pergantian tahun ajaran | Sistem tidak menaikkan kelas atau menetapkan status akademik otomatis. Impor daftar target membuat histori penempatan baru berdasarkan NISN exact; murid lama tanpa penempatan target hanya masuk daftar pemeriksaan read-only dan tidak memblokir aktivasi keseluruhan. |
