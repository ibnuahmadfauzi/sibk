...
| Aplikasi BK    | Ruang kerja layanan BK        | Kasus, penanganan, konsultasi mandiri, klasifikasi tindak lanjut, penyelesaian, prestasi minimum, laporan, dan audit. |
...
| GOV-01  | Sistem harus mendukung Koordinator BK sebagai penanggung jawab operasional.                               | P0       | Menu operasional penugasan, verifikasi, dan rekap tersedia bagi Koordinator tanpa membuka hak edit catatan profesional. |
...
| MD-06  | Admin IT harus dapat mengimpor daftar minimum dari URL API Siswa publik atau CSV UTF-8 fallback dengan field exact `nisn,nama,rombel,tahun_pelajaran` ke satu atau beberapa tahun ajaran yang belum aktif. | P0 | Setiap tahun pelajaran harus sudah dibuat sebagai data sementara; URL hanya dipakai sekali dan tidak disimpan; seluruh respons/berkas divalidasi sebelum diproses; hasil membuat atau memperbarui hanya baris yang disebutkan tanpa menyimpan payload mentah atau menonaktifkan baris lain. |
...
| MD-08  | Koordinator BK harus dapat mengaktifkan tahun ajaran pada atau setelah tanggal mulai ketika data dan penugasan lengkap. | P0 | Target belum aktif; periode valid dan sedang berlaku; minimal satu rombel aktif; setiap rombel memiliki minimal satu keanggotaan murid aktif; setiap murid hanya memiliki satu keanggotaan aktif pada tahun target; dan setiap rombel memiliki tepat satu Guru BK aktif yang mencakup tanggal mulai. **Perlu Konfirmasi**, asal data sementara, dan identitas sementara yang belum selesai hanya menjadi peringatan. |
...
| CASE-03 | Kasus harus memuat informasi awal, penanganan awal, satu jenis tindak lanjut terkini, dan penyelesaian. | P0 | Tidak ada riwayat/jadwal tindak lanjut atau data koordinasi aplikasi. |
...
| CASE-05 | Guru BK harus dapat memilih atau mengosongkan satu jenis tindak lanjut terkini. | P0 | Nilai non-null mengubah kasus menjadi Tindak Lanjut; nilai kosong mengembalikan Sedang Proses; kasus Selesai tidak berubah lewat aksi ini. |
...
| CASE-09 | Jenis tindak lanjut disimpan sebagai satu `follow_up_type_id` nullable pada kasus. | P0 | Nilai harus berasal dari reference aktif kategori `follow_up_type`; tidak memiliki status atau hasil sendiri. |
...
| CASE-17 | Edit kasus resmi hanya menerima `initial_info`, `initial_action`, `resolution_summary`, `expected_updated_at`, dan `action=save|complete`. | P0 | `complete` mewajibkan Catatan Penyelesaian; konflik `updated_at` ditolak tanpa menimpa perubahan lain. |
...
| CONS-01 | Konsultasi adalah record mandiri yang terhubung ke murid atau identitas sementara. | P0 | Field wajib adalah tanggal, jenis layanan, permasalahan, penanganan, hasil, dan Guru BK; masing-masing narasi maksimal 10.000 karakter. |
| CONS-02 | Daftar laporan Waka hanya memuat field layanan yang disetujui dan tidak menyediakan ekspor. | P0 | Policy dan proyeksi server membatasi keluaran ke Ringkasan, Guru BK, Keterangan, serta identitas layanan yang disetujui; pembacaan Waka diaudit. |
...
| CONS-04 | Konsultasi mandiri menyimpan tepat satu murid/identitas sementara, `service_field_id`, `session_date`, `problem`, `handling`, `result`, dan `counselor_id`. | P0 | Konsultasi tidak terhubung ke kasus, tidak memiliki status/nomor registrasi, dan edit memakai `expected_updated_at`. |
...
Mapping daftar memakai `resolution_summary` untuk Hasil Permasalahan dan `result` untuk Hasil Konsultasi. Baris detail memakai `initial_info`/`problem` sebagai Latar Belakang Masalah serta `initial_action`/`handling` sebagai Penanganan. Kolom Ringkasan dokumen memakai nilai Hasil yang sama. Kolom Guru BK memakai owner kasus terakhir atau pencatat konsultasi pada `counselor_id`. Keterangan dokumen Permasalahan memakai label `case_source_id` dan label `follow_up_type_id` terbaru dengan tanda `—` bila kosong; Konsultasi ditampilkan `Selesai`. `internal_note` tidak masuk laporan. Implementasi memakai Eloquent, Form Request, Blade, Bootstrap, SCSS, JavaScript ringan yang sudah ada, dan Laravel Pagination tanpa dependency tabel baru.

Rekap ditandatangani Koordinator BK dan Waka Kesiswaan. Dokumen kasus ditandatangani owner kasus terakhir dan Waka; konsultasi memakai `counselor_id` dan Waka. Pengguna login tidak menjadi fallback. Koordinator/Waka hanya dipilih bila tepat satu akun aktif tersedia; kondisi kosong/ganda ditampilkan sebagai penandatangan belum tersedia. Kepala Sekolah dan NIP tidak ditampilkan karena belum mempunyai sumber data pada schema saat ini. Blok tanda tangan hanya muncul di akhir preview cetak, tidak pada Excel, serta tidak boleh terpotong page break.
...
|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------|
| Kasus               | Kode internal, murid/identitas sementara, sumber, tanggal layanan, bidang, informasi awal, penanganan awal, `follow_up_type_id`, Catatan Penyelesaian, Guru BK, status, waktu dibuat, catatan internal, dan `closed_at`. | `follow_up_type_id` nullable dari reference aktif; kode tidak masuk keluaran pengguna; objek harus berada dalam kewenangan; waktu dibuat otomatis. |
| Tindak lanjut       | Satu `follow_up_type_id` pada kasus. | Nilai nullable mengklasifikasikan tindak lanjut terkini; tidak memiliki jadwal, status, hasil, atau record anak. |
| Konsultasi          | Tepat satu murid/identitas sementara, tanggal, jenis layanan, permasalahan, penanganan, hasil, dan Guru BK. | `problem`, `handling`, dan `result` wajib, maksimal 10.000 karakter; tidak terhubung kasus dan tidak memiliki status/nomor registrasi/catatan privat terpisah. |
...
| Referensi e-Tatib   | ID sumber, NISN/nama/kelas sumber, waktu kejadian, jenis, kategori, poin kejadian, total poin resmi, pencatat, waktu revisi, dan tombstone.                 | Hanya-baca; raw payload tidak disimpan.                      |
| Prestasi            | Murid, jenis, tingkat, kegiatan, penyelenggara, tanggal, hasil, bukti, status verifikasi.                                                                  | P0 bertahap.                                                 |
| Proses keluar murid | Murid, jenis keluar, status, tanggal pencatatan, ringkasan rekomendasi, tanggal efektif, pencatat, pemutus, waktu keputusan, dan catatan keputusan.                  | Satu baris per murid; `effective_date` wajib hanya untuk `resmi_keluar`; narasi maksimal 500 karakter. |
...
- Tindak lanjut adalah satu klasifikasi terkini pada kasus, bukan riwayat kegiatan atau jadwal.
...
- Kesiapan aktivasi diblokir oleh periode yang belum/sudah tidak berlaku, rombel tanpa murid aktif, penempatan aktif ganda pada tahun target, atau rombel tanpa tepat satu Guru BK aktif. **Perlu Konfirmasi**, asal sementara, dan rekonsiliasi tertunda hanya menjadi peringatan.
...
- Status kasus memakai tiga kode aktif; jenis tindak lanjut dan verifikasi prestasi tetap sebagai data referensi terpisah.
...
| Pemisahan tugas                | Hak teknis, Waka hanya-baca, dan scope Guru BK tidak saling memperluas otomatis.              |
...
| Fallback     | Bila Dapodik terlambat 2–3 bulan, Admin IT menyiapkan tahun ajaran dan CSV minimum seluruh murid aktif berupa `nisn,nama,rombel,tahun_pelajaran` berdasarkan dasar resmi sekolah; satu berkas dapat memuat beberapa tahun yang sudah dibuat; Koordinator mengaktifkan setelah data/penugasan lengkap dan tanggal mulai tiba. |
...
| DEP-03 | Istilah layanan    | Nama, kardinalitas, kewajiban, dan pemilik data referensi selain tiga status kasus dan jenis tindak lanjut. | Label form/filter lain dapat berubah. |
