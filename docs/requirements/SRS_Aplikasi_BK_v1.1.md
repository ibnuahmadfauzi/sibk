...
| MD-05  | Admin IT harus dapat membuat tahun ajaran operasional tanpa memasukkan tanggal mulai atau tanggal selesai. | P0 | Tahun ajaran baru menyimpan nama/identitas tahun dan selalu dibuat `is_active=false`; waktu serta actor aktivasi dicatat ketika Koordinator mengaktifkannya. |
| MD-06  | Admin IT harus dapat mengimpor roster minimum melalui Excel `.xlsx` ke tahun ajaran target. | P0 | Setiap baris minimal memuat NISN, nama, dan rombel; seluruh berkas divalidasi sebelum pratinjau, berkas mentah tidak disimpan, dan tidak ada perubahan master sebelum Admin IT mengonfirmasi penerapan. |
...
| INT-05  | Admin IT dapat mengelola konfigurasi endpoint dan credential Dapodik/e-Tatib tanpa mengubah kode atau file environment.          | P0          | Konfigurasi bersifat tingkat sekolah/provider dan tetap berlaku lintas tahun ajaran; hanya diubah ketika endpoint, credential, kontrak, atau kebijakan berubah; secret tidak pernah ditampilkan kembali. |
...
| Kasus               | Kode internal, murid/identitas sementara, sumber, tanggal layanan, bidang, `initial_info`, `initial_action`, `resolution_summary` nullable, Guru BK, status, waktu dibuat, dan `closed_at`. | Label UI narasi adalah Latar Belakang Masalah, Penanganan, dan Hasil / Ringkasan; kode tidak masuk keluaran pengguna; `resolution_summary` wajib hanya saat penyelesaian. |
...
| Konsultasi          | Tepat satu murid/identitas sementara, tanggal, jenis layanan, `problem`, `handling`, `result` nullable, dan Guru BK. | `problem` dan `handling` wajib; `result` dapat diisi kemudian; setiap narasi maksimal 10.000 karakter; tidak memiliki status/nomor registrasi. |
...
| Mirror e-Tatib      | Identitas sumber, NISN, kelas sumber bila tersedia, tanggal/waktu kejadian, jenis pelanggaran, kategori, poin, pencatat, total poin provider bila tersedia, dan waktu sinkronisasi. | Hanya-baca; tidak menjadi sumber perubahan e-Tatib dan tidak memicu lifecycle kasus otomatis. |
| Prestasi            | Murid, jenis, tingkat, kegiatan, penyelenggara, tanggal, dan hasil. | Dikelola Waka; tanpa bukti/lampiran dan tanpa status verifikasi. |
...
| NFR-13 | Impor Excel roster/prestasi dan penerapan hasil Dapodik harus divalidasi penuh sebelum mutasi dan diproses secara atomik. | P0 | Berkas `.xlsx` hanya dipakai selama proses import, seluruh baris divalidasi sebelum transaksi, NISN menjadi kunci roster, dan satu kesalahan/konflik/kegagalan membatalkan seluruh perubahan; batas ukuran/baris mengikuti kontrak implementasi yang diuji. |
...
| Data minimum           | Identitas sumber, NISN, kelas sumber bila tersedia, waktu kejadian, pelanggaran, kategori, poin, pencatat, total poin provider bila tersedia, dan waktu sinkronisasi. |
| Penggunaan Guru BK     | Cek Poin Pelanggaran membaca mirror sesuai scope; data e-Tatib dapat ditautkan ke kasus tanpa diketik ulang. |
| Penanda                | Data baru sejak terakhir dilihat dapat diberi label **Data e-Tatib Baru**; penanda hanya informatif dan tidak menghitung threshold atau membuat kasus otomatis. |
| Perubahan balik        | Tidak tersedia.                                                                                               |
...
| Impor Excel  | Admin IT dapat mengimpor roster `.xlsx` dari daftar resmi sekolah. Jalur Excel dan API menggunakan aturan pencocokan, pratinjau, konfirmasi, dan penerapan yang sama. |
...
| Riwayat      | Setiap roster tahun target membuat histori penempatan baru; kelas pada layanan/pelanggaran lama tetap merepresentasikan konteks saat kejadian dan tidak ditimpa kelas sekarang. |
...
| 1.1       | 24 September 2026 | Menyelaraskan roster Excel/API Dapodik, aktivasi tahun ajaran tanpa tanggal, mirror/penanda e-Tatib read-only, dua narasi awal layanan, multi-tindak-lanjut, penyelesaian Hasil / Ringkasan, dan prestasi yang dikelola Waka tanpa verifikasi/bukti. |
