...
- **Alur UI e-Tatib aktif (INT-14):** `POST /data-master/etatib/preview` mengembalikan jumlah `rows`, `students`, `matched`, `conflicts`, `missing`, dan `identity_conflicts` berisi nama, NISN, kelas, serta alasan hanya untuk identitas yang perlu diperiksa; rincian pelanggaran tidak ditampilkan. Server menyimpan hash URL, fingerprint respons, pengguna, dan waktu pratinjau di sesi tanpa payload mentah. `POST /data-master/etatib/sync` dan aktivasi otomatis mengambil ulang API dan hanya berlaku bila fingerprint sama dan pratinjau belum lewat 10 menit. Jika `missing > 0`, tombol sinkronisasi nonaktif; server juga menolak sinkronisasi manual maupun terjadwal bila identitas pelanggaran aktif sebelumnya hilang. Konflik identitas dicocokkan di halaman konflik setelah sinkronisasi. URL otomatis hanya disimpan setelah sinkronisasi valid, tidak pernah ditampilkan kembali, dan dihapus saat dinonaktifkan. Alur tidak meminta `source_id` atau token.
