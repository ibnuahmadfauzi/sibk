...
- **Pratinjau:** `ApiSiswaRosterImportService::preview()` mengambil dan memvalidasi respons tanpa membuat audit, `external_sync_runs`, murid, rombel, atau membership. Respons khusus Admin IT memuat jumlah, kesiapan tahun pelajaran, `conflict_count`, dan `entries` hanya untuk baris konflik dengan NISN lengkap, nama, tahun, rombel API, rombel SIBK, serta status. Respons memakai `Cache-Control: no-store, private`. DNS, koneksi ditolak, timeout, SSL, redirect, HTTP gagal, dan JSON tidak valid diberi pesan aman tanpa menampilkan URL. Tombol impor hanya tersedia bila seluruh tahun siap dan tidak ada konflik; import mengambil serta memvalidasi ulang URL yang sama agar data pratinjau tidak perlu disimpan.
