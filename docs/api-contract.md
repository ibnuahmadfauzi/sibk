...
- **Pratinjau:** `ApiSiswaRosterImportService::preview()` mengambil dan memvalidasi respons tanpa membuat audit, `external_sync_runs`, murid, rombel, atau membership. Respons hanya memuat jumlah dan kesiapan tahun pelajaran, tanpa identitas murid. DNS, koneksi ditolak, timeout, SSL, redirect, HTTP gagal, dan JSON tidak valid diberi pesan aman tanpa menampilkan URL. Tombol impor hanya tersedia bila seluruh tahun siap; import mengambil serta memvalidasi ulang URL yang sama agar data pratinjau tidak perlu disimpan.
