...
- **Alur UI e-Tatib aktif:** `POST /data-master/etatib/preview` membaca URL tanpa mutasi dan hanya mengembalikan jumlah `rows`, `students`, `matched`, dan `conflicts`, tanpa contoh identitas/pelanggaran. Admin IT kemudian memilih sinkronisasi sekali pakai atau menyimpan URL terenkripsi untuk jadwal otomatis. Konflik identitas dicocokkan di halaman konflik setelah sinkronisasi. URL otomatis hanya disimpan setelah fetch dan sinkronisasi valid, tidak pernah ditampilkan kembali, dan dihapus saat dinonaktifkan. Payload mentah tidak disimpan. Alur tidak meminta `source_id` atau token.
