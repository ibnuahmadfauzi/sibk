...
- **Alur UI e-Tatib aktif (INT-14):** `POST /data-master/etatib/preview` mengembalikan jumlah `rows`, `students`, `matched`, `conflicts`, `missing`, `missing_students`, `name_mismatches`, `active_year`, dan `identity_conflicts` berisi nama/NISN/kelas sumber, murid master dengan NISN sama (`master`), dan hingga tiga kandidat bernama sama (`suggestions`); rincian pelanggaran tidak ditampilkan. `active_year` hanya informasi tahun ajaran aktif, bukan filter tanggal pelanggaran. Server menyimpan hash URL, fingerprint respons, pengguna, dan waktu pratinjau di sesi tanpa payload mentah. `POST /data-master/etatib/sync` dan aktivasi otomatis menerima `identity_decisions[]` opsional berisi `nisn`, `name`, dan `student_id`; server memeriksa pasangan identitas terhadap snapshot yang diambil ulang dan menyimpan mapping bersama sinkronisasi. Pilihan kosong tetap belum tertaut dan dapat ditangani di halaman konflik. Sinkronisasi hanya berlaku bila fingerprint sama dan pratinjau belum lewat 10 menit. Jika `missing > 0`, tombol sinkronisasi nonaktif; server juga menolak sinkronisasi manual maupun terjadwal bila identitas pelanggaran aktif sebelumnya hilang. URL otomatis hanya disimpan setelah sinkronisasi valid, tidak pernah ditampilkan kembali, dan dihapus saat dinonaktifkan. Alur tidak meminta `source_id` atau token.
