...
- **Alur UI e-Tatib aktif:** `POST /data-master/etatib/preview` membaca URL tanpa mutasi. Admin IT kemudian memilih sinkronisasi sekali pakai atau menyimpan URL terenkripsi untuk jadwal otomatis. URL otomatis hanya disimpan setelah fetch dan sinkronisasi valid, tidak pernah ditampilkan kembali, dan dihapus saat dinonaktifkan. Payload mentah tidak disimpan. Alur tidak meminta `source_id` atau token.
