...
@section('body')
    <div class="sibk-dashboard">
        ...
        <div class="sibk-page-header d-flex flex-wrap justify-content-between gap-3 mb-4">
            ...
            <div class="d-flex gap-2"><button type="button" data-print-report class="btn btn-outline-secondary">Cetak Riwayat</button>@if($canUpdateConsultation)<a href="{{ route('consultations.edit', $consultation) }}" class="btn btn-primary">Edit Data Sesi</a>@endif @if($canArchiveConsultation)<form action="{{ route('consultations.destroy', $consultation) }}" method="POST" data-confirm-submit data-confirm-message="Data akan diarsipkan dan tidak tampil pada daftar utama. Lanjutkan?">@csrf @method('DELETE')<button class="btn btn-outline-danger" type="submit">Hapus</button></form>@endif</div>
        </div>
