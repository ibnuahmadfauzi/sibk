...
@section('body')
    <div class="sibk-dashboard" data-page-id="PG-106">
        <div class="sibk-page-header mb-4"><div class="sibk-page-header__copy"><a href="{{ route('cases.show', $case) }}" class="text-decoration-none small">&larr; Kembali ke detail kasus</a><h1>Selesaikan Kasus</h1><p>{{ $case->identityName() }} &bull; {{ $case->service_date->locale('id')->translatedFormat('d F Y') }}</p></div></div>
        ...
        <div class="sibk-panel">
            <form action="{{ route('cases.resolve', $case) }}" method="POST" class="row g-4 p-4">
                ...
                <div class="col-12"><label class="form-label" for="waka_summary">Ringkasan Penanganan untuk Waka <span class="text-danger">*</span></label><textarea class="form-control" id="waka_summary" name="waka_summary" rows="4" maxlength="500" required>{{ old('waka_summary', $case->waka_summary) }}</textarea><div class="form-text">Tuliskan hasil umum secara faktual tanpa percakapan konseling atau catatan pribadi.</div></div>
