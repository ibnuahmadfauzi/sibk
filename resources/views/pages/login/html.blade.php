        ...
        <div class="sibk-auth-card card border-0">
            ...
            <form id="loginForm" action="{{ route('login.store') }}" method="post" novalidate>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group @error('email') @if($message !== 'Email atau kata sandi tidak sesuai.') is-invalid @endif @enderror">
                        <span class="input-group-text" aria-hidden="true">
                            <svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4.4 3.6-8 8-8s8 3.6 8 8"/></svg>
                        </span>
                        <input class="form-control @error('email') @if($message !== 'Email atau kata sandi tidak sesuai.') is-invalid @endif @enderror" id="identifier" name="email" type="email" required
                            autocomplete="username" spellcheck="false"
                            value="{{ old('email') }}" placeholder="Masukkan email akun"
                            aria-describedby="identifierError">
                    </div>
                    @error('email')
                        @if($message !== 'Email atau kata sandi tidak sesuai.')
                ...
                <div class="mb-4">
                    ...
                    <div class="input-group @error('password') is-invalid @enderror @if($errors->first('email') === 'Email atau kata sandi tidak sesuai.') is-invalid @endif">
                        <span class="input-group-text" aria-hidden="true">
                            <svg viewBox="0 0 24 24"><rect x="5" y="10" width="14" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>
                        </span>
                        <input class="form-control @error('password') is-invalid @enderror @if($errors->first('email') === 'Email atau kata sandi tidak sesuai.') is-invalid @endif" id="password" name="password" type="password" required
